...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        int row = place.row, col = place.col;
        if (isCell(row - 1, col)) {
            Place up = pl(row - 1, col);
            action.accept(up);
        }
        if (isCell(row + 1, col)) {
            Place down = pl(row + 1, col);
            action.accept(down);
        }
        if (isCell(row, col - 1)) {
            Place left = pl(row, col - 1);
            action.accept(left);
        }
        if (isCell(row, col + 1)) {
            Place right = pl(row, col + 1);
            action.accept(right);
        }
    ...
    final boolean solved() {
        if (activeRegionSize() == _width * _height) {
            return true;
        }
        return false;
    }
    ...
    Set<Place> adjacentCells(int color) {
        ...
        HashSet<Place> newActive = new HashSet<Place>();
        for (Place place : _active) {
            newActive.add(place);
        }
        undo();
        ...
        for (Place place : newActive) {
            if (!(_active.contains(place))) {
                adjacent.add(place);
            }
        }
        return adjacent;
