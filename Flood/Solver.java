...
class Solver {
    ...
    static int[] chooser(Model model, int depth) {
        ...
        bestMove = -1;
        maxSize = -1;
        int numColors = model.ncolors(), currentColor = 0;
        Model currentModel = new Model(model);

        for (; currentColor < numColors; currentColor += 1) {
            int size = currentModel.adjacentCells(currentColor).size();
            if (size > maxSize) {
                maxSize = size;
