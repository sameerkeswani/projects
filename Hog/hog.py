...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k <= num_rolls:
        ...
        if dice_num == 1:
            contains_one = True
        k, total = k + 1, total + dice_num
    return 1 if contains_one else total
    # END PROBLEM 1
...
def first_two_digits(n):
    """Return first two digits of number
    >>> first_two_digits(192)
    19
    """
    while n // 10 >= 10:
        n = n // 10
...
def more_boar(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    player_digits = first_two_digits(player_score)
    opponent_digits = first_two_digits(opponent_score)
    
    player_second_digit = player_digits % 10
    opponent_second_digit = opponent_digits % 10
    player_first_digit = player_digits // 10
    opponent_first_digit = opponent_digits // 10

    if player_second_digit < opponent_second_digit and player_first_digit < opponent_first_digit:
        return True
    else:
        return False
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 0:
            ...
            if current_score - last > highest_increase:
                highest_increase = current_score - last
                print("Player 0 has reached a new maximum point gain.", highest_increase, "point(s)!")
                
        elif who == 1:
            current_score = score1
            if current_score - last > highest_increase:
                highest_increase = current_score - last
                print("Player 1 has reached a new maximum point gain.", highest_increase, "point(s)!")
        
        last = current_score
        
        return announce_highest(who, last, highest_increase)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    k = 1
    highest_average, dice_number = 0, 0
    while k <= 10:
        calculate_average = make_averaged(roll_dice, trials_count)
        current_average = calculate_average(k, dice)
        if current_average > highest_average:
            highest_average, dice_number = current_average, k     
        k += 1
    return dice_number
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    potential_score = score + piggy_points(opponent_score)
    
    if more_boar(potential_score, opponent_score):
        return 0
    elif piggypoints_strategy(potential_score, opponent_score, cutoff, num_rolls) == 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
