...
def choose(paragraphs, select, k):
    ...
    for para in paragraphs:
        if select(para):
            possible_selections = possible_selections + [para]

    if k < len(possible_selections):
        return possible_selections[k]
    else:
        return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    # END PROBLEM 2
    def select(phrase):
        words = split(lower(remove_punctuation(phrase)))
        check_contain = False
        for each_topic in topic:
            for each_word in words:
                if each_word == each_topic:
                    check_contain = True
...
def accuracy(typed, reference):
    ...
    if num_typed == 0 and num_ref == 0:
        return 100.0
    elif num_typed == 0 or num_ref == 0:
        return 0.0
    else:
        correct = [1 for i in range(min(num_typed, num_ref)) if typed_words[i] == ref_words[i]]
        return (sum(correct)/num_typed) * 100 
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        if diff_function(typed_word, word, limit) <= limit:
            differences += [diff_function(typed_word, word, limit)]
            words_in_limit += [word]

    
    combined_list = list(zip(words_in_limit, differences))
    
    if not(differences):
        return typed_word
    else:
        return min(combined_list, key = lambda word: word[1])[0]
    # END PROBLEM 5
...
def sphinx_switches(start, goal, limit):
    ...
    if start == goal:
        ...
        return sphinx_switches(start[0:len(goal)], goal, limit - 1) + (len(start) - len(goal))
    elif len(start) < len(goal):
        return sphinx_switches(start, goal[0:len(start)], limit - 1) + (len(goal) - len(start))
    else:
        if start[0] == goal[0]:
            return sphinx_switches(start[1:len(start)], goal[1:len(goal)], limit)
        else:
            return 1 + sphinx_switches(start[1:len(start)], goal[1:len(goal)], limit - 1)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, send):
    ...
    send({'id': user_id, 'progress':num_correct/len(prompt)})
...
def time_per_word(times_per_player, words):
    ...
    for i in times_per_player:
        new_list = [[i[j+1] - i[j] for j in range(0, len(i) - 1)]]
        times += new_list
...
def fastest_words(game):
    ...
    for i in word_indices: 
        ...
        for j in player_indices:
            if time(game, fastest_index, i) > time(game, j, i):
                fastest_index = j

        fastest_words[fastest_index].append(word_at(game, i))
