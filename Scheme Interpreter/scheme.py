...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        evaluated_operands = rest.map(lambda subexpr: scheme_eval(subexpr, env))
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while temp is not nil:
            arguments_list.append(temp.first)
            temp = temp.rest
        if self.use_env:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest is not nil:
        ...
        if eval_first is nil or eval_first is False:
            return eval_first
        else:
            expressions = expressions.rest
    return scheme_eval(expressions.first, env)
...
def do_or_form(expressions, env):
    ...
    while expressions.rest is not nil:
        eval_first = scheme_eval(expressions.first, env)
        if eval_first or (eval_first == 0 and not is_false_primitive(eval_first)):
            return eval_first
        expressions = expressions.rest
    return scheme_eval(expressions.first, env)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while temp_bindings is not nil:
        binding = temp_bindings.first
        validate_form(binding, 2, 2)
        temp_bindings = temp_bindings.rest
    
    names = bindings.map(lambda x: x.first)
    values = bindings.map(lambda y: scheme_eval(y.rest.first, env))
