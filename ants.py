...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while check_place != beehive:
            if check_place.bees and (self.min_range <= count_place <= self.max_range):
                return bee_selector(check_place.bees)
            else:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        hungry_place = self.place

        if self.chewing > self.chew_duration:
            self.chewing = 0
        elif self.chewing > 0:
            self.chewing += 1
        if self.chewing == 0 and hungry_place.bees:
            hungry_place.bees[0].reduce_health(hungry_place.bees[0].health)
            self.chewing += 1
# END Problem 7
...
# END Problem EC
    ...
    def __init__(self, health=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(health)
        QueenAnt.num_queens.append(self)
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self is not QueenAnt.num_queens[0]:
            self.reduce_health(self.health)
        else:
            super().action(gamestate)
            exit_place = self.place.exit
            while exit_place:
                if not exit_place.ant:
                    exit_place = exit_place.exit
                else:
                    exit_place.ant.buff()  
                    if exit_place.ant.is_container() and exit_place.ant.contained_ant:
                        exit_place.ant.contained_ant.buff()
                    exit_place = exit_place.exit
        # END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health(amount)
        if self.health == 0 and self is QueenAnt.num_queens[0]:
            bees_win()
        # END Problem EC

    def remove_from(self, place):
        if self is QueenAnt.num_queens[0]:
            self = self
        else:
